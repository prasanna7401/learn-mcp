...
class MCPClient:
    ...
    ### STEP - 2:
    # Implement a method to convert the list of root directory paths into a list of `Root` objects that can be sent to the server. 
    # Each `Root` object should contain a URI (Uniform Resource Identifier) that points to the root directory.
    ...
    ### STEP - 3:
    # Implement a callback method to handle the `list_roots` request from the server.
    # This method should return a `ListRootsResult` object containing the list of `Root` objects created from the provided root directory paths.
